...
const checkpasswordconf = document.querySelector("form input[name='passwordConf']");

function removeContainedClass(element, classname) {
    element.classList.contains(classname) ? element.classList.remove(classname) : null;
}
...
function checkUsernameEmailOrPassword() {
    ...
    if ((checkpassword.value != this.value) && (this.value != "") && (this.name === "passwordConf")) {
...
if (checkemail && checkusername && checkpasswordconf) {
    [checkemail, checkusername, checkpasswordconf].forEach((formelement, index) => {
